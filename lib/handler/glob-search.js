...
module.exports = new Handler({
  ...
  toRoute(options, rc) {
    ...
    function walk(pathname, cwd, glob, result, resultCounter) {
      ...
      if (stat.isDirectory()) {
        ...
        lang.each(files, (filename) => {
          walk(path.join(pathname, filename), cwd, glob, result, resultCounter);
        });
      }
    }
    ...
    return (req, res, next) => {
      ...
      if (req._handler === name) {
        ...
        if (!query) {
          ...
          const result = [];
          walk(normalizedPathname, normalizedPathname, query, result, 0);
          res._JSONRes({
            paths: result
          });
        }
