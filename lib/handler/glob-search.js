...
module.exports = new Handler({
  ...
  toRoute(options, rc) {
    ...
    function walk(pathname, glob, result, resultCounter) {
      if (resultCounter >= options.maxPathsCount) return;
      ...
      if (stat.isDirectory()) {
        ...
        lang.each(files, (filename) => {
          walk(path.join(pathname, filename), glob, result, resultCounter);
        });
      }
    }
    ...
    return (req, res, next) => {
      ...
      if (req._handler === name) {
        ...
        if (!query) {
          ...
          const result = [];
          console.log(req._normalizedPathname, query);
          walk(req._normalizedPathname, query, result, 0);
          res._JSONRes({
            paths: result
          });
        }
