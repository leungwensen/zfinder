...
module.exports = new Handler({
  ...
  toRoute(options, rc) {
    ...
    function walk(pathname, query, queryRegExp, result, resultCounter, allowDotfiles) {
      ...
      if (stat.isDirectory()) {
        ...
        lang.each(files, (filename) => {
          walk(path.join(pathname, filename), query, queryRegExp, result, resultCounter, allowDotfiles);
        });
      } else if (stat.isFile()) {
        if (isBinary(pathname) || stat.size > options.maxFileSize) {
          return;
        }
        ...
        lang.some(str.split('\n'), (line, i) => {
          ...
          if (linesCounter === options.maxLinesCount) {
            return true;
          }
          return false;
        });
    ...
    return (req, res, next) => {
      ...
      if (req._handler === name) {
        ...
        if (!query) {
          ...
          const result = [];
          walk(req._normalizedPathname, query, queryRegExp, result, 0, req._allowDotfiles);
          res._JSONRes({
            files: result
          });
        }
