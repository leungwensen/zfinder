...
module.exports = (rc) => {
  ...
  server
    ...
    .use((req, res, next) => { // pre-handlers
      ...
      if (!req._allowDotfiles && isHidden(req._normalizedPathname)) {
        return next(httpErrors(400));
      }
      ...
      try {
        ...
        req._pathInfo = wrapPathInfoWithProps(
          root,
      ...
      // isBinary
      req._isBinary = isBinary(pathname);

      // language
      req._fileLanguage = getLanguageByFilename(pathname);
      ...
      if (partsOfPathname[1] === rc.path.handler && partsOfPathname[2]) {
        req._handler = partsOfPathname[2];
      }

      // raw handler (mode)
      if (lang.hasKey(params, '_raw')) {
        req._raw = true;
      }
