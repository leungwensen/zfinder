...
module.exports = function (config) {
    ...
    // FIXME body is not available until you set the correct headers
        ...
        .use(function (req, res, next) {
            ...
            var pathname = path.normalize(path.join(root, decodeURIComponent(urlInfo.pathname)));
    ...
    // process middlewares {
    var middlewares = lang.extend({}, config.middleware, config.localmiddleware);
    var routes = [];
    lang.forIn(middlewares, function (options, mwName) {
        options = options || {};
        var mw;
        try {
            utils.log(colors.grey(
        ...
        if (mw && mw.getRoutes) {
            lang.each(mw.getRoutes(), function (route) {
    ...
    // apply middlewares
    lang.each(routes, function (route) {
        console.log(route);
